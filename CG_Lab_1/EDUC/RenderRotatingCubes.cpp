...
RenderRotatingCubes::RenderRotatingCubes()
{
...
bool RenderRotatingCubes::Init(HWND hwnd)
{
	HRESULT hr = S_OK;
	ID3DBlob* pVSBlob = NULL;
	
	hr = CompileShaderFromFile("solar.hlsl", "VS", "vs_5_0", &pVSBlob );
	...
	hr = CompileShaderFromFile( "solar.hlsl", "PS", "ps_5_0", &pPSBlob );
	if(FAILED(hr)) return false;
...
bool RenderRotatingCubes::Draw()
{
	...
	immediate_context_->UpdateSubresource( m_pConstantBuffer, 0, NULL, &cb1, 0, 0 );
	...
	immediate_context_->UpdateSubresource( m_pConstantBuffer, 0, NULL, &cb2, 0, 0 );
...
void RenderRotatingCubes::Update()
{
	static float t = 0.0f;
	...
	m_World1 = DirectX::XMMatrixRotationY( t );
...
void RenderRotatingCubes::Close()
{
